...
- **Mobile** doesn't support OPDS catalogs yet — support is planned for `1.5.0`.
- **Desktop** doesn't support HTML catalogs, passphrase discovery, or cover handling yet.
:::
...
Load it either as a module import, installed from npm (registers the elements as a side effect):
...
or with a plain `<script src>` — no install, no build step. Since the package is also published to npm, the same bundle is available from unpkg or jsDelivr with no need to host it yourself:
